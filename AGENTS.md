...
- After the user approves a production plan containing phone actions, the Twilio control plane starts one `pycloudflared` tunnel, verifies its public `/api/health` endpoint, and reuses a task-scoped lease across that plan's call queue. Startup is not eager. The Private Workspace honestly reports `Establishing a secure connection…`; readiness failure places no call and offers retry or plan revision. Per-call webhook URLs stream bidirectional PCMU audio through Twilio Media Streams and OpenAI Realtime. Every call gets separate high-entropy voice, status, and media capability tokens bound to its approved task, Twilio Account SID, and Call SID. A missing Twilio signature is tolerated only with the correct scoped capability; a present invalid signature is rejected. Capabilities are revoked at terminal call status and never logged.
