...
- There is no hosted Relay backend, shared account system, or multi-tenant state. Every install uses only that user's credentials, SQLite data, event logs, and task-scoped local tunnel.
- Approved production plans can execute phone-call actions only when the planner has collected the caller's preferred display name and supplies a valid E.164 number, concrete known facts, and honest contact provenance; researched contacts require a source URL, while user-provided contacts do not. Speaker is the only live audio model and sees representative audio plus approved context. A separate low-latency text Gatekeeper owns missing-fact decisions and routes every private dashboard message. Exact short acknowledgements bypass Gatekeeper deterministically, and model classifications receive only approved call facts/document context rather than raw planning history. Raw private text never enters Speaker's conversation: the backend may refresh Speaker only with a typed, append-only confirmed context update, waits for `session.updated`, and then permits `response.create`. Private/meta messages receive a reply only in the Private Workspace. While `WAITING_FOR_USER`, representative audio continues to be transcribed; normal Speaker output pauses, but the backend may trigger a constrained one-line keep-alive at intervals. Calls return to the Private Workspace after the queue finishes.
- When a production plan containing phone actions is first shown for review, the Twilio control plane starts one background `pycloudflared` preparation and verifies its public `/api/health` endpoint. It does not start at bare application startup. Approval awaits that same bounded preparation, whose readiness timeout defaults to 90 seconds and is configurable with `RELAY_TUNNEL_READINESS_TIMEOUT`. The task-scoped lease is reused across the approved call queue. The Private Workspace honestly reports `Establishing a secure connection…` at the dial boundary; readiness failure places no call and offers retry or plan revision. Per-call webhook URLs stream bidirectional PCMU audio through Twilio Media Streams and OpenAI Realtime. Every call gets separate high-entropy voice, status, and media capability tokens bound to its approved task, Twilio Account SID, and Call SID. A missing Twilio signature is tolerated only with the correct scoped capability; a present invalid signature is rejected. Capabilities are revoked at terminal call status and never logged.
