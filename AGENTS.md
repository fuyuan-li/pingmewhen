...
- Relay renders structured quick replies and context-sensitive inputs using prebuilt components, including yes/no buttons, date/month pickers, and masked numeric identifiers.
...
- Secure mode mutes/disconnects the cloud AI and pauses transcription. Each requested payment, SSN, or date-of-birth field forces a typed takeover and local-TTS cycle before Relay can resume; the user types the real value themselves and it never reaches the cloud model.
...
- The Call Console has an off-by-default, receive-only Listen control. It monitors the representative and Relay through a best-effort browser audio tap; it never captures a microphone or sends browser audio into the call. Protected-mode audio is excluded from the monitor.
