...
- Approved production plans can execute phone-call actions only when the planner supplies a valid E.164 number and honest contact provenance; researched contacts require a source URL, while user-provided contacts do not. The task engine dials each action sequentially through Twilio, starts a separate OpenAI Realtime session for each representative, injects private live instructions, persists both transcript sides, and returns to the Private Workspace after the queue finishes.
