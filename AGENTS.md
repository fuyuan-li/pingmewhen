...
- Approved production plans can execute phone-call actions only when the planner has collected the caller's preferred display name and supplies a valid E.164 number and honest contact provenance; researched contacts require a source URL, while user-provided contacts do not. Each Realtime call opens as an outbound caller on that person's behalf, discloses Relay is an AI once, injects private live instructions, and persists both transcript sides. When a fact is missing, a structured Realtime tool moves the call to `WAITING_FOR_USER`, displays the question through the existing call prompt, gates both audio directions, and resumes the same session after the user's typed answer. Calls return to the Private Workspace after the queue finishes.
